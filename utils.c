...
void makeMove(Board* board, move mov) {
	...
	if ((sq0 == A1 || sq1 == A1) && (board->castle & C_WQ)) {
		board->castle &= (~C_WQ);
		board->hash ^= pieceHashes[BP][C_WQ - 1];
	} else if ((sq0 == H1 || sq1 == H1) && (board->castle & C_WK)) {
		board->castle &= (~C_WK);
		board->hash ^= pieceHashes[BP][C_WK - 1];
	...
	switch (p) {	//special logic for special pieces
		...
		case BK:
			...
			while (bit <= m2) {		//update our hash value.
				if (m2 & bit) {
					board->hash ^= pieceHashes[BP][bit-1];
				}
				bit <<= 1;
			}
			board->castle &= (~mask);
	...
	if (board->enpas != NO_SQUARE)	{
	board->hash ^= pieceHashes[WP][board->enpas % 8];	//update enpassant status in the hash.
	}
	if (enpas != NO_SQUARE) {
		board->hash ^= pieceHashes[WP][enpas % 8];
	}
	board->enpas = enpas;
	board->ply++;
